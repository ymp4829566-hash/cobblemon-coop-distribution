...
현재 FancyMenu 클라이언트 리소스에는 타이틀부터 서버 접속 화면까지 이어지는 `필드 6` 메뉴 BGM이 포함됩니다. 재생은 FancyMenu Universal Audio 요소가 담당하며 반복 재생, 음악 채널, 기본 음량 30%로 설정되어 있습니다. 코옵 야생전 모드 JAR에는 메뉴 음악 코드나 음원이 포함되지 않습니다.
